...
//  Booking Requests
if (isset($_POST['bookingRequests'])  && isset($_SESSION['RentalAgency'])) {
    $RentalAgency = $_SESSION['RentalAgency'];
    $LIMIT = 10;
    $selectBookingList = "
        SELECT * FROM `carBookings` WHERE `status` = '0' AND `carBookings`.`vehicleNo`  IN (SELECT `vehicleNo` FROM `vehicleList` WHERE `vehicleList`.`userId` = '$RentalAgency')
    ";
    if (isset($_POST['ShowRows'])) {
        if ($_POST['ShowRows'] == "") {
            $LIMIT = 10;
